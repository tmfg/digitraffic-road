...
public class CameraPresetHistoryDataService {
    ...
    @Transactional(readOnly = true)
    public List<CameraHistoryDto> findCameraOrPresetPublicHistory(final List<String> cameraOrPresetIds,
     ...
     */
    @Transactional(readOnly = true)
    public CameraHistoryChangesDto findCameraOrPresetHistoryChangesAfter(final Instant after,
        ...
        final List<PresetHistoryChangeDto> changes =
                cameraPresetHistoryRepository.findCameraPresetHistoryChangesAfter(after, cameraIds, presetIds);
    ...
    private void checkAllParametersUsedAndNotTooLong(final List<String> cameraOrPresetIds,
                                                     final List<String> usedCameraIds,
                                                     final List<String> usedPresetIds) {

        checkCameraOrPresetIdsMaxSize(cameraOrPresetIds);

        final List<String> illegalIds =
                cameraOrPresetIds.stream().filter(id -> !usedCameraIds.contains(id) && !usedPresetIds.contains(id))
                        .collect(Collectors.toList());

        if (!illegalIds.isEmpty()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Parameter camera or presetId should be either 6 or 8 chars long. Illegal parameters: %s.",
     ...
     */
    @Transactional(readOnly = true)
    public CameraHistoryPresencesDto findCameraOrPresetHistoryPresences(final String cameraOrPresetId,
        ...
        if (cameraOrPresetId == null) {
            ...
            return findCameraPresetHistoryPresences(cameraOrPresetId, checkAndFixFromTime(fromTime),
                    checkAndFixToTime(toTime));
        } else if (isCameraId(cameraOrPresetId)) {
            return findCameraHistoryPresences(cameraOrPresetId, checkAndFixFromTime(fromTime),
                    checkAndFixToTime(toTime));
        } else {
            throw new IllegalArgumentException(
                    String.format("Parameter cameraOrPresetId should be either 6 or 8 chars long. Was %d long.",
    ...
    private CameraHistoryPresencesDto findCameraHistoryPresences(final Instant fromTime, final Instant toTime) {
        final List<PresetHistoryPresenceDto> presetsHistoryStatuses =
                cameraPresetHistoryRepository.findCameraPresetHistoryPresenceByTime(fromTime, toTime,
                        getOldestTimeLimit());
        return convertToCameraHistoryPresences(presetsHistoryStatuses, fromTime, toTime);
    }

    private CameraHistoryPresencesDto findCameraHistoryPresences(final String cameraId, final Instant fromTime,
                                                                 final Instant toTime) {
        ...
        final List<PresetHistoryPresenceDto> history =
                cameraPresetHistoryRepository.findCameraPresetHistoryPresenceByCameraIdAndTime(cameraId, fromTime,
                        toTime, getOldestTimeLimit());
        return convertToCameraHistoryPresences(history, fromTime, toTime);
    }

    private CameraHistoryPresencesDto findCameraPresetHistoryPresences(final String presetId, final Instant fromTime,
                                                                       final Instant toTime) {
        if (!cameraPresetHistoryRepository.existsByIdPresetId(presetId)) {
            throw new ObjectNotFoundException("CameraHistory", presetId);
        }
        final List<PresetHistoryPresenceDto> history =
                cameraPresetHistoryRepository.findCameraPresetHistoryPresenceByPresetIdAndTime(presetId, fromTime,
                        toTime, getOldestTimeLimit());
        return convertToCameraHistoryPresences(history, fromTime, toTime);
    }

    private static CameraHistoryPresencesDto convertToCameraHistoryPresences(
            final List<PresetHistoryPresenceDto> presetsHistoryPresences,
            final Instant fromTime, final Instant toTime) {

        final Map<String, List<PresetHistoryPresenceDto>> cameraIdToPresetHistoryPresences =
                presetsHistoryPresences.parallelStream()
                        .collect(Collectors.groupingBy(PresetHistoryPresenceDto::getCameraId));

        final List<CameraHistoryPresenceDto> result =
                cameraIdToPresetHistoryPresences.entrySet().stream()
                        .map(e -> new CameraHistoryPresenceDto(e.getKey(), e.getValue()))
    ...
    public HistoryStatus resolveHistoryStatusForVersion(final String presetImageName, final String versionId) {
        ...
        final CameraPresetHistory history = findHistoryVersionInclSecretInternal(
                WeathercamS3Properties.getPresetIdFromImageName(presetImageName), versionId);
    ...
    private List<String> parseCameraIds(final List<String> cameraOrPresetIds) {
        ...
        return cameraOrPresetIds.stream().filter(CameraPresetHistoryDataService::isCameraId)
                .collect(Collectors.toList());
    }
    ...
    private List<String> parsePresetIds(final List<String> cameraOrPresetIds) {
        ...
        return cameraOrPresetIds.stream().filter(CameraPresetHistoryDataService::isPresetId)
                .collect(Collectors.toList());
    }
