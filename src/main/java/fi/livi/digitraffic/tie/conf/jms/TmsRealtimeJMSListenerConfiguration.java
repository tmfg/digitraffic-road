...
public class TmsRealtimeJMSListenerConfiguration extends AbstractJMSListenerConfiguration<LAMRealtimeProtos.Lam> {
    ...
    @Autowired
    public TmsRealtimeJMSListenerConfiguration(@Qualifier("sonjaJMSConnectionFactory") QueueConnectionFactory connectionFactory,
                                       @Value("${jms.userId}") final String jmsUserId, @Value("${jms.password}") final String jmsPassword,
                                       @Value("${jms.tms.realtime.inQueue}") final String jmsQueueKey, final SensorDataUpdateService sensorDataUpdateService,
