        ...
        if (mqttMessageSender.acquireLock()) {
            try {
                final Pair<Instant, List<TrafficAnnouncementFeature>> trafficMessagesPair =
                    trafficMessageMqttDataServiceV1.findTrackingsForMqttCreatedAfter(mqttMessageSender.getLastUpdated());
                LOGGER.debug("method=pollAndSendMessages.TRAFFIC_MESSAGE count={}", trafficMessagesPair.getRight().size());
                ...
                if(!trafficMessagesPair.getRight().isEmpty()) {
                    ...
                    LOGGER.debug("method=pollAndSendMessages.TRAFFIC_MESSAGE sending count={}", dataMessages.size());
