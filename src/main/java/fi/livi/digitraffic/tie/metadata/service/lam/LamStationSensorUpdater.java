...
@Service
public class LamStationSensorUpdater extends AbstractRoadStationSensorUpdater {
    private static final Logger log = LoggerFactory.getLogger(LamStationSensorUpdater.class);

    private final LotjuLamStationClient lotjuLamStationClient;

    @Autowired
    public LamStationSensorUpdater(final RoadStationSensorService roadStationSensorService,
    ...
    /**
     * Updates all available lam road station sensors
     */
    @Transactional
    public boolean updateRoadStationSensors() {
        log.info("Update LAM RoadStationSensors start");

        if (lotjuLamStationClient == null) {
            log.warn("Not updating LAM stations sensors because no lotjuLamStationClient defined");
            return false;
        }
    ...
    private static boolean updateRoadStationSensorAttributes(final LamLaskennallinenAnturiVO from, final RoadStationSensor to) {
        ...
        to.setRoadStationType(RoadStationType.LAM_STATION);
