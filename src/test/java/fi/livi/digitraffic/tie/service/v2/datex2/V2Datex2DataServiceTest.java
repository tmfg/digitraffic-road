...
public class V2Datex2DataServiceTest extends AbstractServiceTest {
    ...
    public void activeIncidentsDatex2AndJsonEquals() throws IOException {
        ...
        final List<ImsGeoJsonFeature> activeJsons = v2Datex2DataService.findActiveJson(0, Datex2MessageType.TRAFFIC_INCIDENT);
        ...
        final ImsGeoJsonFeature situationJson = activeJsons.get(0);

        final JsonMessage jsonProperties = situationJson.getProperties();
        Assert.assertEquals(GUID, situation.getId());
        Assert.assertEquals(GUID, jsonProperties.getSituationId());
        ...
        final ZonedDateTime situationJsonStart = jsonProperties.getReleaseTime();
        ...
        final String descJson = jsonProperties.getAnnouncements().get(0).getLocation().getDescription();
        final String titleJson = jsonProperties.getAnnouncements().get(0).getTitle();
    ...
    public void findBySituationId() throws IOException {
        ...
        final List<ImsGeoJsonFeature> jsons = v2Datex2DataService.findAllBySituationIdJson(GUID, Datex2MessageType.TRAFFIC_INCIDENT);
        ...
        final ImsGeoJsonFeature situationJson = jsons.get(0);

        Assert.assertEquals(GUID, situation.getId());
        Assert.assertEquals(GUID, situationJson.getProperties().getSituationId());
    }
