...
CREATE OR REPLACE PROCEDURE create_modified_at_trigger_on_all_tables()
  LANGUAGE PLPGSQL
AS
$$
DECLARE
...
BEGIN
FOR _sql IN
SELECT 'DROP TRIGGER IF EXISTS ' || table_name || '_modified_at_t on ' || table_name || '; ' ||
       'CREATE TRIGGER ' || table_name || '_modified_at_t BEFORE UPDATE ON ' || table_name ||
       ' FOR EACH ROW EXECUTE PROCEDURE update_modified_at_column();' AS drop_and_create_trigger_query
FROM information_schema.columns
WHERE table_schema = 'public'
...
call create_modified_at_trigger_on_all_tables();
